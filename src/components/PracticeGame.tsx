  ...
  const generateSimulatedVideo = async (textToDraw: string): Promise<Blob | null> => {
    return new Promise((resolve) => {
      try {
        ...
        const interval = setInterval(() => {
          frame++;
          ctx.fillStyle = '#0B1F3A'; // Dark luxury canvas background
        ...
        <div className="flex flex-col justify-between h-full flex-1">
          <div>
            <div className="flex items-center gap-1.5 mb-6" id="game-setup-header">
              <button onClick={onBackToCelebration} className="text-[#2563EB] hover:text-[#1D4ED8] flex items-center gap-1 text-xs font-mono font-medium cursor-pointer">
                <ArrowLeft size={14} /> Back
              </button>
            </div>

            <div className="text-center mb-6">
              <span className="text-xs font-mono font-bold text-[#2563EB] uppercase tracking-widest">DRILLED CONVICTION</span>
              <h2 className="text-2xl font-serif font-bold text-slate-900 mt-1">Spontaneity Drill</h2>
            ...
            {qnaPairs.length === 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 text-center" id="no-reflections-error">
                ...
                <h4 className="text-sm font-semibold text-slate-800">No Journal Entries Found</h4>
                <p className="text-xs text-slate-600 mt-2 px-4 leading-normal">
              ...
              <div className="flex flex-col gap-4 mt-4" id="setup-qna-selection">
                <h3 className="text-xs font-mono text-slate-400 uppercase tracking-wider pl-1">Choose your prompt</h3>
                {qnaPairs.map((pair, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleStartDrill(idx)}
                    className="bg-white hover:bg-slate-50 text-left rounded-2xl p-5 border border-slate-200 hover:border-[#2563EB] transition-all cursor-pointer group active:scale-99 shadow-xs"
                    id={`practice-prompt-option-${idx}`}
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-[10px] font-mono text-slate-400 font-semibold uppercase">Inquiry {idx + 1}</span>
                      <ArrowRight size={14} className="text-slate-300 group-hover:text-[#2563EB] transition-colors" />
                    </div>
                    <h4 className="text-sm font-serif font-bold text-slate-800 mt-2 leading-snug line-clamp-2">
                      {pair.question}
                    </h4>
                    <p className="text-xs italic text-slate-500 mt-2 line-clamp-2 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                      "{pair.answer}"
                    </p>
                  </button>
                ))}
      ...
      {gamePhase === 'flash' && currentQna && (
        <div className="flex flex-col justify-between h-full flex-1 text-center" id="game-flash-view">
          <div>
            ...
            <h3 className="text-xl font-serif font-bold text-slate-900 px-4 leading-snug mt-6">
              {currentQna.question}
            </h3>
          </div>

          {/* Core high-contrast prompt & written answer to absorb */}
          <div className="my-8 bg-[#0B1F3A] rounded-3xl p-6 md:p-8 text-left shadow-xl relative overflow-hidden" id="flash-card">
            <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest font-bold">Your Written Compass</span>
            <p className="text-white font-serif text-base italic leading-relaxed mt-3">
              "{currentQna.answer}"
            </p>
            <div className="absolute right-[-15px] bottom-[-15px] opacity-10 text-[#2563EB]">
              <Camera size={100} />
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="relative w-20 h-20 flex items-center justify-center rounded-full bg-[#EFF5FE] border-4 border-amber-300">
              <span className="text-2xl font-mono font-bold text-slate-800">{timeLeft}</span>
              <div className="absolute inset-0 rounded-full border-2 border-[#2563EB]/20 animate-ping pointer-events-none" />
            </div>
      ...
      {gamePhase === 'record' && currentQna && (
        <div className="flex flex-col justify-between h-full flex-1" id="game-record-view">
          ...
          <div className="text-center mb-4">
            ...
            <h3 className="text-sm font-serif font-bold text-slate-800 px-4 line-clamp-1">
              {currentQna.question}
            </h3>
          </div>

          {/* Viewfinder Container */}
          <div className="bg-slate-900 rounded-3xl overflow-hidden aspect-video relative flex items-center justify-center border-4 border-rose-500/30 shadow-2xl" id="viewfinder">
            ...
            {!hasCameraAccess && (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-slate-950 text-slate-400">
                <VideoOff size={40} className="text-[#2563EB] mb-3 animate-pulse" />
            ...
            <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-xs px-3 py-1.5 rounded-xl border border-white/10 max-w-[80%]">
              <p className="text-[9px] font-mono text-[#2563EB] uppercase font-bold">Target Duration</p>
          ...
          <div className="mt-6 flex justify-center" id="game-controls">
            <button
              ...
              className="bg-[#0B1F3A] hover:bg-[#2563EB] text-white font-sans font-medium py-3 px-6 rounded-2xl transition-all shadow-md active:scale-98 flex items-center gap-1.5 text-sm cursor-pointer"
        ...
        <div className="flex flex-col justify-between h-full flex-1" id="game-summary-view">
          <div>
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-serif font-bold text-slate-900 mt-1">Conviction Programmed</h2>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 mb-6 shadow-xs" id="recordings-library">
              <h3 className="text-xs font-mono text-slate-400 uppercase tracking-wider pl-1 mb-3">Recorded Spontaneity Rounds</h3>
                  ...
                  {recordings.map((clip, idx) => (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                        activePlaybackUrl === clip.url ? 'border-amber-400 bg-amber-50/20' : 'border-slate-100 bg-slate-50'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className="bg-slate-200 text-slate-800 p-2 rounded-lg text-xs font-mono font-bold">
                          R{clip.round}
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-800">Round {clip.round} Speak</p>
                          <p className="text-[10px] text-slate-400 font-mono">Limit: {clip.duration}s</p>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="bg-white hover:bg-slate-100 border border-slate-200 text-[#2563EB] p-2 rounded-xl transition-all shadow-2xs cursor-pointer"
                        title="Play recording"
                      >
                        <Play size={14} className="fill-[#2563EB]" />
                      </button>
                    </div>
                  ))}
                </div>
              ...
              {activePlaybackUrl && (
                <div className="mt-4 bg-slate-900 rounded-2xl overflow-hidden aspect-video relative flex items-center justify-center" id="playback-viewport">
                  <video
            ...
            {/* prompt summary */}
            <div className="bg-[#EFF5FE] rounded-2xl p-4 border border-[#D6E4FA] text-[11px] text-slate-600 font-serif italic leading-relaxed" id="summary-closing-quote">
          ...
          <div className="mt-8 flex flex-col gap-3" id="summary-footer">
            <button
              ...
              className="w-full bg-[#0B1F3A] hover:bg-[#2563EB] disabled:bg-slate-300 text-white font-sans font-medium py-3.5 px-6 rounded-2xl transition-all shadow-md active:scale-98 flex items-center justify-center gap-1.5 cursor-pointer"
            ...
            <button
              onClick={handleReset}
              className="text-xs text-slate-400 hover:text-slate-700 font-mono py-1 cursor-pointer flex items-center justify-center gap-1 mx-auto"
